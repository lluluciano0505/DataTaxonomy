...
with left_kpi:
    ...
    c2.metric("High Risk",     int((filtered["risk_level"] == "High").sum()))
    c3.metric("Medium Risk",   int((filtered["risk_level"] == "Medium").sum()))
    c4.metric("Low Risk",      int((filtered["risk_level"] == "Low").sum()))
...
with col4:
    ...
    risk_domain = filtered.groupby(["domain","risk_level"]).size().reset_index(name="count")
    risk_color  = {"High":"#e74c3c","Medium":"#f39c12","Low":"#2ecc71"}
    if not risk_domain.empty:
        fig_risk = px.bar(risk_domain, x="count", y="domain", color="risk_level",
...
show_cols = ["filename","domain","scale","lifecycle","asset_type","confidentiality",
             "governance","risk_level","confidence","year","short_summary"]
